...
pub fn initialize(&mut self, treasury: AccountId, dex_contract_id: AccountId, admin: AccountId, fee_bps: u32) {
    ...
    self.dex_contract.insert(U64(0), dex_contract_id);
    self.admin = admin;
    ...
    pub fn swap(
        ...
        assert!(amount_in_u128 > U128(0), "Amount must be positive");
        assert!(min_amount_out_u128 > U128(0), "Minimum output must be positive");
        ...
        log!("🔄 Swap #{}, TX: {}", expected.0, tx_hash);
        log!("From: {} -> To: {}", input_token, output_token);
        log!("Amount: {}, Min Out: {}", amount_in_u128.0, min_amount_out_u128.0);

        let dex_contract = self.dex_contract.get(&U64(0)).expect("No DEX contract configured").clone();
        ...
        let quote_promise = Promise::new(dex_contract.clone())
            .function_call(
                ...
                json!({
                    ...
                    "amount_in": amount_in_u128.0.to_string(),
                })
                .to_string()
                .as_bytes()
                .to_vec(),
    ...
    pub fn execute_swap_after_quote(
        ...
        let expected_output: U128 = if quote_str.contains("amount_out") {
            ...
            if let Some(amount_str) = v["amount_out"].as_str() {
                U128(amount_str.parse::<u128>().unwrap_or(0))
            } else {
                U128(0)
            }
        } else {
            // Fallback: extract number from string
            U128(quote_str.split(':')
                .last()
        ...
        assert!(
            expected_output.0 >= min_amount_out_u128.0,
            "Quote insufficient: {} < {}",
            expected_output.0,
            min_amount_out_u128.0
        );
        
        // SECURITY: Apply dynamic slippage check
        let max_slippage_amount = U128(
            expected_output.0
        ...
        let min_acceptable_output = expected_output.0
            .checked_sub(max_slippage_amount.0)
            .unwrap();
        
        assert!(
            min_acceptable_output >= min_amount_out_u128.0,
        ...
        let transfer_promise = Promise::new(input_token.clone())
            .function_call(
                ...
                json!({
                    ...
                    "amount": amount_in_u128.0.to_string(),
                    "memo": format!("Swap #{}", nonce.0),
                })
                .to_string()
                .as_bytes()
                .to_vec(),
            ...
            Promise::new(dex_contract.clone())
                .function_call(
                    ...
                    json!({
                        ...
                        "output_token": output_token.clone(),
                        "amount_in": amount_in_u128.0.to_string(),
                        "min_amount_out": min_amount_out_u128.0.to_string(),
                        "receiver_id": contract_id.clone(),
                    })
                    .to_string()
                    .as_bytes()
    ...
    pub fn transfer_to_user_with_fees(
        ...
        let actual_output: U128 = U128(balance_result.parse::<u128>().unwrap_or(0));
        ...
        let fee_amount: U128 = if self.fee_bps > 0 {
            U128(
                actual_output.0
        ...
        let user_amount = actual_output.0.checked_sub(fee_amount.0).unwrap();
            ...
            Some(
                Promise::new(output_token.clone()).function_call(
                    ...
                    json!({
                        "receiver_id": user_id.clone(),
                        "amount": user_amount.to_string(),
                        "memo": format!("Swap #{}, TX: {}", self.nonce.0, tx_hash),
                    })
                    .to_string()
                    .as_bytes(),
                    NearToken::from_yoctonear(0), 
        ...
        let transfer_fee = if fee_amount > U128(0) {
            Some(
                Promise::new(output_token.clone()).function_call(
                    "ft_transfer".to_string(), 
                    json!({
        ...
        log!("✅ Swap #{} completed successfully", nonce.0);
        log!("   User: {}, Received: {} {}", user_id, user_amount, output_token);
        log!("   Fee: {} {}", fee_amount.0, output_token);
        log!("   TX: {}", tx_hash);
    ...
    pub fn log_final_result(&mut self, tx_hash: String) -> () {
        ...
        log!("🎉 Swap finalized.");
        log!("Nonce committed: {}", self.nonce.0);
        log!("TX: {}", tx_hash);
    }
    ...
    pub fn update_dex_contract(&mut self, new_dex: AccountId) {
        self.assert_admin();
        self.dex_contract.insert(U64(0), new_dex);
        log!("DEX contract updated to: {}", self.dex_contract.get(&U64(0)).expect("No DEX contract configured"));
    }
    ...
    pub fn add_whitelisted_token(&mut self, token: AccountId) {
        ...
        if exists.is_none() {
            ...
            self.whitelisted_tokens_count = U64(self.whitelisted_tokens_count.0 + 1);
        ...
        Promise::new(token)
            .function_call(
                ...
                json!({
                    ...
                    "amount": amount_u128.0.to_string(),
    ...
    pub fn get_status(&self) -> Value {
        json!({
            ...
            "dex_contract": self.dex_contract.get(&U64(0)).map(|c| c.to_string()).unwrap_or_else(|| "Not set".to_string()),
            "admin": self.admin,
...
mod tests {
    ...
    fn test_secure_initialization() {
        ...
        assert_eq!(contract.nonce, U64(0));
    }
